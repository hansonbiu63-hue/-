...
def parse_args():
    ...
    p.add_argument("--threshold_mode", choices=["fixed", "global"], default="global")
    p.add_argument("--threshold_min", type=float, default=0.10)
    p.add_argument("--threshold_max", type=float, default=0.90)
    p.add_argument("--threshold_step", type=float, default=0.02)
    p.add_argument("--patience", type=int, default=8)
    p.add_argument("--min_delta", type=float, default=1e-4)
    p.add_argument("--label_cleaning", choices=["none", "target_intersection"], default="target_intersection")
    p.add_argument("--min_target_hit_frames", type=int, default=3)
    p.add_argument("--min_target_hit_ratio", type=float, default=0.02)
    p.add_argument("--file_pattern", default="Frame_*.csv")
...
def select_clean_unity_label(window, window_aoi_ids, base, args):
    """Return a cleaner window-level multilabel target for Unity finetune data.

    The raw task label usually describes all target controls in the current task
    round. A 3-second sliding window, however, may contain evidence for only a
    subset of those controls. With target_intersection enabled, we keep only task
    targets that are actually hit by the gaze AOI sequence inside this window.
    """
    task_y = select_unity_label(window, base)
    if args.label_cleaning == "none" or task_y.sum() <= 0:
        return task_y

    task_ids = set(np.where(task_y > 0.5)[0].tolist())
    valid_aoi = window_aoi_ids[(window_aoi_ids >= 0) & (window_aoi_ids < AOI_COUNT)]
    clean_y = np.zeros(AOI_COUNT, dtype=np.float32)
    if len(valid_aoi) == 0:
        return clean_y

    denom = max(1, len(valid_aoi))
    for aid in task_ids:
        hit_frames = int(np.sum(valid_aoi == aid))
        hit_ratio = hit_frames / denom
        if hit_frames >= args.min_target_hit_frames and hit_ratio >= args.min_target_hit_ratio:
            clean_y[aid] = 1.0
    return clean_y
...
def load_samples(args):
    ...
    if args.stage == "pretrain":
        ...
        files = sorted(Path(args.frame_dir).rglob(args.file_pattern))
        if args.max_files:
            files = files[: args.max_files]
        for path in files:
...
def compute_stats(samples):
    """Compute feature normalization stats in a memory-safe streaming way.

    The previous implementation concatenated all overlapping windows into one
    huge array before computing mean/std. For dense sliding-window data, this can
    easily require several GiB of RAM. Here we only accumulate sum and squared
    sum over continuous features, so the peak memory stays close to one window.
    """
    ...
    sum_vec = np.zeros(cont_dim, dtype=np.float64)
    sumsq_vec = np.zeros(cont_dim, dtype=np.float64)
    count = 0

    for sample in samples:
        # Shape: [window_frames, len(CONTINUOUS_IDXS)].
        # Keep copy=False to avoid duplicating each window when possible.
        x_cont = sample.x[:, CONTINUOUS_IDXS].astype(np.float64, copy=False)
        sum_vec += x_cont.sum(axis=0)
        sumsq_vec += (x_cont * x_cont).sum(axis=0)
        count += x_cont.shape[0]

    if count <= 0:
        return mean, std

    cont_mean = sum_vec / float(count)
    cont_var = (sumsq_vec / float(count)) - cont_mean ** 2
...
def export_random_forest_json(path, args, models, metrics):
    ...
    payload = {
        ...
        "aoi_count": AOI_COUNT,
        "threshold": float(threshold if threshold is not None else args.threshold),
        "threshold_mode": args.threshold_mode,
        "label_cleaning": args.label_cleaning,
        "min_target_hit_frames": args.min_target_hit_frames,
        "min_target_hit_ratio": args.min_target_hit_ratio,
        "metrics": metrics,
        "forests": forests,
    }
...
@torch.no_grad()
def collect_probs_targets(model, loader, args):
    model.eval()
    ...
    return torch.cat(probs_list, 0).numpy(), torch.cat(targets_list, 0).numpy()


def multilabel_metrics_np(y_true, probs, threshold):
    ...
    subset = float(np.mean(np.all(pred == y_true, axis=1)))
    tp = (pred * y_true).sum(axis=0)
    fp = (pred * (1.0 - y_true)).sum(axis=0)
    fn = ((1.0 - pred) * y_true).sum(axis=0)
    precision = tp / (tp + fp + 1e-6)
    recall = tp / (tp + fn + 1e-6)
    ...
    for i in range(y_true.shape[0]):
        truth = np.where(y_true[i] > 0.5)[0].tolist()
        hit3.append(any(t in top3[i].tolist() for t in truth))
...
def find_best_global_threshold(y_true, probs, args):
    best_thr = float(args.threshold)
    best_metrics = multilabel_metrics_np(y_true, probs, best_thr)
    best_score = best_metrics["macro_f1"]

    # Include max value despite floating point step accumulation.
    thresholds = np.arange(args.threshold_min, args.threshold_max + args.threshold_step * 0.5, args.threshold_step)
    for thr in thresholds:
        metrics = multilabel_metrics_np(y_true, probs, float(thr))
...
def write_unity_stats(path, args, mean, std, samples, threshold=None):
...
def train(args):
    ...
    print(f"[INFO] loaded samples: train={len(train_s)}, val={len(val_s)}, test={len(test_s)}")
    print(f"[INFO] label_cleaning={args.label_cleaning}, threshold_mode={args.threshold_mode}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.stage == "pretrain":
            ...
            if args.threshold_mode == "global":
                val_probs, val_targets = collect_probs_targets(model, val_loader, args)
                current_threshold, val = find_best_global_threshold(val_targets, val_probs, args)
            else:
                val = evaluate(model, val_loader, args)
                current_threshold = float(args.threshold)
            metric = val["macro_f1"]
        ...
        if metric > best_metric + args.min_delta:
            ...
            if args.patience > 0 and patience_left <= 0:
                print(f"Early stopping triggered at epoch={epoch}. best_metric={best_metric:.6f}, best_threshold={best_threshold:.4f}")
                break
    ...
    if args.stage == "finetune":
        test_probs, test_targets = collect_probs_targets(model, test_loader, args)
        test = multilabel_metrics_np(test_targets, test_probs, best_threshold)
        test["best_threshold"] = best_threshold
        test["best_metric"] = float(ckpt.get("best_metric", best_metric))
        test["best_epoch"] = int(ckpt.get("best_epoch", -1))
    else:
    ...
    if args.stage == "finetune":
        write_unity_stats(
            Path(args.stats_json) if args.stats_json else out_dir / "eye_transformer_stats.json",
            args,
            mean,
            std,
            samples,
